$(document).ready(function() {
    ...
    function getGeolocation() {
        const geo = fetch("https://ipapi.co/json/")
            .then(data => data.json())
            .then(json => {
                // console.log(JSON.stringify(json, null, 2));
                useGeolocationInfo(json);
            })
            .catch(error => {
                console.error(error);
    ...
    function useGeolocationInfo(json) {
        ...
        if (info.lat && info.lon) {
            fetch(`https://api.openweathermap.org/data/2.5/weather?appid=e005b47d860a06d7e4e18d731fa3e7a4&lat=${info.lat}&lon=${info.lon}&units=metric`)
